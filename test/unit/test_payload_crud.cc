...
static std::string NewTempPath() {
    char tmpl[] = "/tmp/astervec_crud_XXXXXX";
    char* dir = mkdtemp(tmpl);
    REQUIRE(dir != nullptr);
    return std::string(dir);
}

static std::unique_ptr<astervec::AsterVecDB> OpenFresh(std::string* out_path) {
    *out_path = NewTempPath();
    ...
    REQUIRE(astervec::AsterVecDB::Open(*out_path, opts, &db).ok());
    return db;
}
...
TEST_CASE("GetPayload: id without metadata returns empty object") {
    ...
    REQUIRE(db->Insert(1, astervec::Span<float>(v)).ok());   // no metadata
...
TEST_CASE("GetPayload: id with metadata returns stored JSON") {
    ...
    auto db = OpenFresh(&path);
    std::vector<float> v{1,2,3,4};
    REQUIRE(db->Insert(1, astervec::Span<float>(v), R"({"k":"v"})").ok());
...
TEST_CASE("SetPayload: overwrites existing metadata") {
    ...
    auto db = OpenFresh(&path);
    std::vector<float> v{1,2,3,4};
    REQUIRE(db->Insert(1, astervec::Span<float>(v), R"({"k":"v1"})").ok());
...
TEST_CASE("SetPayload: invalid JSON rejected") {
    ...
    auto db = OpenFresh(&path);
    std::vector<float> v{1,2,3,4};
    REQUIRE(db->Insert(1, astervec::Span<float>(v)).ok());
...
TEST_CASE("UpdatePayload: merge-patch adds fields") {
    ...
    auto db = OpenFresh(&path);
    std::vector<float> v{1,2,3,4};
    REQUIRE(db->Insert(1, astervec::Span<float>(v), R"({"a":1})").ok());
...
TEST_CASE("UpdatePayload: null deletes field") {
    ...
    auto db = OpenFresh(&path);
    std::vector<float> v{1,2,3,4};
    REQUIRE(db->Insert(1, astervec::Span<float>(v), R"({"a":1,"b":2})").ok());
...
TEST_CASE("DeletePayloadKeys: removes keys, idempotent for missing") {
    ...
    auto db = OpenFresh(&path);
    std::vector<float> v{1,2,3,4};
    REQUIRE(db->Insert(1, astervec::Span<float>(v), R"({"a":1,"b":2,"c":3})").ok());

    std::vector<std::string> keys{"b", "nonexistent"};
    REQUIRE(db->DeletePayloadKeys(1, astervec::Span<const std::string>(keys)).ok());
...
TEST_CASE("DeletePayloadKeys: NotFound on id with no metadata") {
    ...
    auto st = db->DeletePayloadKeys(1, astervec::Span<const std::string>(keys));
    CHECK(st.IsNotFound());
    db->Close();
    std::filesystem::remove_all(path);
}
